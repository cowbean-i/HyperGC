...
class Decoder(nn.Module):
    ...
    def __init__(self, hidden_dim, ncell, lstm_layers,cell_type):
        """
        Initiate Decoder
        ...
        if cell_type=="rnn":
            self.input_to_hidden = nn.Linear(hidden_dim, hidden_dim)
            self.hidden_to_hidden = nn.Linear(hidden_dim, hidden_dim)
        elif cell_type=="gru":
            self.input_to_hidden = nn.Linear(hidden_dim, 3 * hidden_dim)
            self.hidden_to_hidden = nn.Linear(hidden_dim, 3 * hidden_dim)
        elif cell_type=="lstm":
    ...
    def forward(self, n,decoder_input,hidden,s,hopmask,edge_size,top_p,cell_type):
        ...
        # Break the loop if no active sequences remain
            ...
            if cell_type=="rnn":
                hi = torch.tanh(self.input_to_hidden(decoder_input) + self.hidden_to_hidden(h))
                c = None
            elif cell_type=="gru":
                gates = self.input_to_hidden(decoder_input) + self.hidden_to_hidden(h)
                z, r, h_tilda = gates.chunk(3, 1)

                z = torch.sigmoid(z)      # update gate
                r = torch.sigmoid(r)      # reset gate
                h_tilda = torch.tanh(h_tilda + r * h)  # candidate hidden
            ...
            #indices = masked_outs.argmax(dim=-1)  
...
class GenCL(nn.Module):
    ...
    def generator(self, n: Tensor ,e: Tensor, s: list, edge_size: list, hopmask, top_p, cell_type):
...
#c0=torch.randn((num_genhe,self.dec_hid_dim),device=n.device)
        ...
        (outputs, pointers), decoder_hidden, sorted_idx = self.decoder(n,decoder_input0,decoder_hidden0,s, hopmask,edge_size,top_p,cell_type)
