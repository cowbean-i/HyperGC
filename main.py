...
from HyperGC.models import HyperEncoder,Decoder,GenCL
...
def train(args):
    ...
    masked_nodes1,start1,edge_size1,drop_idx1=drop_hyperedges(hyperedge_index,num_nodes,num_edges,args.drop_hyperedge_rate,args.ncell,args.node_select,args.quan)
    masked_nodes2,start2,edge_size2,drop_idx2=drop_hyperedges(hyperedge_index,num_nodes,num_edges,args.drop_hyperedge_rate,args.ncell,args.node_select,args.quan)
    ...
    for i in range(num_batches):
        ...
        prob1, gen_e1, sorted_idx1, z1 = model.generator(n1,e1,start1,edge_size1,hopmask,args.top_p,args.cell_type)
        prob2, gen_e2, sorted_idx2, z2 = model.generator(n2,e2,start2,edge_size2,hopmask,args.top_p,args.cell_type)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('HyperGC unsupervised learning.')
    ...
    parser.add_argument('--quan', type=float, default=1.0)
    ...
    if args.task=="edge":
        ...
        for seed in range(args.num_seeds):
            data.edge_split=data.load_splits(seed)
            
            encoder = HyperEncoder(data.features.shape[1], args.hid_dim, args.hid_dim, args.num_layers)
            decoder = Decoder(args.hid_dim,args.ncell,args.lstm_layers,args.cell_type)
        ...
        print(f'data:{args.data}, result: {t_auroc_mean*100:.1f} ± {t_auroc_std*100:.1f}\n')
    ...
    if args.task=="node":
        ...
        if args.cluster:
            ...
            print(f'data:{args.data}, result: {t_acc_mean*100:.1f} ± {t_acc_std*100:.1f}\n')
